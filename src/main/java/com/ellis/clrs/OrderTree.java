...
public class OrderTree {
  ...
  public static int totalfloor(BTree root) {
  ...
  public static String printTimes(String s, int time) {
    StringBuilder sb = new StringBuilder();
  ...
  public static String printWidth(String s, int width) {
    if (s == null) {
  ...
  public static void printTree(BTree root, List<Point> points, int row, int col, int floors) {
    if (root != null) {
  ...
  public static int squat(int s, int b) {
    if (b == 0) return 1;
    int result = 1;
    for (int i = 0; i < b; i++) {
      result *= s;
  ...
  public static void lastOrder(BTree root) {
    if (root == null) return;
    lastOrder(root.left);
  ...
  // 左边小，右边大，相等时放左边
  public static void createOrder(int[] a, BTree root) {
  ...
  public static void insert(BTree root, BTree leaf) {
